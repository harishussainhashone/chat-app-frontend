...
export default function VisitorDetailPage() {
  ...
  const rawId = Array.isArray(params?.visitorId) ? params.visitorId[0] : params?.visitorId;
  const visitorId = Number(rawId);
  const chat = chats.find((item) => item.id === visitorId);
  const [activeTab, setActiveTab] = useState(2);

  if (!chat) return <div>Visitor not found</div>;
