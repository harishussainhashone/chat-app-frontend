...
export default function ChatsTabBar({
  activeTab = 2,
  onTabChange,
  onAddTab,
  onCloseHeader,
    ...
    <div className="w-full bg-gray-100">
      <div className="flex">
        <div className="flex-1">
          ...
          <div className="flex bg-surface-2 my-2 mx-5 border-gray-200 items-end">
            {tabs.map((tab, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center h-8 px-4 text-sm font-medium ${
                    isActive
                      ? "bg-surface text-gray-900 border border-gray-300 border-b-surface relative -mb-px"
                      : "bg-surface-3 text-gray-600 hover:text-gray-900 hover:bg-gray-200 border border-transparent"
                  } ${!isLast ? "border-r-0" : ""}`}
                  style={{ zIndex: isActive ? 10 : 1 }}
                >
                  {tab.label}
                  {tab.badge && (
                    <span className="ml-2 px-2 py-0.5 text-xs rounded bg-red-500 text-white font-semibold">
                      Open
                    </span>
                  )}
                </button>
